...
function queryMenuList(){//获取职务列表
	var menucode = $("#menucode").val();
	var menuname = $("#menuname").val();
	var obj = {"menucode":menucode,"menuname":menuname,"pageIndex":page,"pageSize":limit};
	$.ajax({
		url:url+"/ACL_MenuData/queryMenuListByPage",
		type:"POST",
		data:obj,
		dataType:"json",
		async:false,//很重要
		success:function(data){
			...
			var txt1="";
			 if(list.findIndex(target=>target.Code=="update")==-1&&window.top.arr.length!=0){
				 ...
				 txt="<a href='menu_update.html' class='layui-btn layui-btn-xs updata' lay-event='edit'>编辑</a>";
			  }
			...
			$.each(data.result,function(i,val){
				...
				html+="<tr><td class='no-print'><span class='checkbox' style='float:none'></span></td><td>"+menucode
				+"</td><td>"+menuname
				+"</td><td>"+menupath
				+"</td><td>"+menuimg
				+"</td><td>"+parentid
				+"</td><td>"+orderid
				+"</td><td>"+remark
				+"</td><td class='center no-print' data-id='"+id+"'>"+txt+txt1+"</td></tr>";
			})
			$("#cont").append(html);	
		}
	})
