	...
	layer.confirm("确定分析？",{title:"提示信息"},function(index){
		...
		$.ajax({
			...
			success:function(data){
				layer.close(index);
				if(data.flag){
					layer.msg("分析成功，请点击统计分析",{time:2000});
				}else{
	...
	layer.confirm("确定分析？",{title:"提示信息"},function(index){
		...
		$.ajax({
			...
			success:function(data){
				layer.close(index);
				if(data.flag){
					layer.msg("分析成功，请点击搜索",{time:2000});
				}else{
					layer.msg(data.reason,{time:2000});
				}
			},
...
$(document).on("click",".watch",function(){
	...
	var year=$("#year").val();
	var month=$(".month").val();
	var daKa=$(this).parent().siblings().eq(3).html();
	var normal=$(this).parent().siblings().eq(4).html();
	var yc=$(this).parent().siblings().eq(5).html();
	var queQin=$(this).parent().siblings().eq(6).html();
	var chiDao=$(this).parent().siblings().eq(7).html();
	var early=$(this).parent().siblings().eq(8).html();
	var kuangGong=$(this).parent().siblings().eq(9).html();
	var holiday=$(this).parent().siblings().eq(10).html();
	var business=$(this).parent().siblings().eq(11).html();
	localStorage.holderno=holderno;
	localStorage.year=year;
	localStorage.month=month;
	localStorage.normal=normal;
	localStorage.daKa=daKa;
	localStorage.queQin=queQin;
	localStorage.chiDao=chiDao;
	localStorage.early=early;
	localStorage.kuangGong=kuangGong;
	...
	$.ajax({
		...
		success:function(data){
			...
			if(data.flag){
				...
				for(var item of res){
					var $tr=$("<tr></tr>");
					var $td1=$("<td>"+item.holderno+"</td>");
					var $td2=$("<td>"+item.holdername+"</td>");
					var $td3=$("<td>"+item.departmentname+"</td><td>"+data.days+"</td>");
					var $td4=$("<td></td>");
					var $td5=$("<td></td>");
					var $td6=$("<td></td>");
					var $td7=$("<td></td>");
					...
					if(list.findIndex(target=>target.Code=="query")==-1&&window.top.arr.length!=0){
						$a=$("<a href='javascript:;' class='layui-bg-gray layui-btn layui-btn-xs layui-btn-warm'>查看</a>");
					  }else {
					...
					for(var i=0;i<item.child.length;i++){
						if(item.child[i].status=="正常"){
							...
							$td10.html(item.child[i].frequency);
						}else if(item.child[i].status=="出差"){
							$td11.html(item.child[i].frequency);
						}else if(item.child[i].status=="异常"){
							$td12.html(item.child[i].frequency);
						}
					}
					$tr.append($td1)
					$tr.append($td2);
		...
		btn2:function(index,layero){
			...
			$.ajax({
				...
				success:function(data){
					...
					if(data.flag){
						var res=data.result;
						var $table = $("<table id='tableData' style='display:none'><tr><td>工号</td><td>姓名</td><td>部门</td><td>应打卡(天)</td><td>正常(天)</td>" +
								"<td>异常(天)</td><td>休息(天)</td><td>迟到(天)</td><td>早退(天)</td><td>旷工(天)</td><td>请假(天)</td><td>出差(天)</td></tr></table>");
							for(var item of res){
								var $tr=$("<tr></tr>");
