...
function showHide(){
	...
	$("#save").hide();
	$("#printInsertSpareParts_SDData").hide();
	var list=[];
	arrList.forEach(item=>{
		if(item.ModelCode=="13003"){
			list.push(item);
		}
	})
	for(var item of list){
		...
		if(item.Code=="print"){
			$("#printInsertSpareParts_SDData").show();
		}
	}
...
$.ajax({
		url:url+"/SpareParts_SDData/insertSparePartsSDDataBatch",
		type:"POST",
...
$(".itemcode").bind("keypress",function(event){
    ...
    if(event.keyCode == "13")
    {
    	querySparePartsData(itemcode,this);
    }
});
//根据入库页面中的原材料条码查询数据
function querySparePartsData(itemcode,that){
	...
	$.ajax({
		url:url+"/SparePartsData/querySparePartsData",
		type:"POST",
		data:obj,
		success:function(data){
			if(!data.flag){
