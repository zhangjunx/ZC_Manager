$(function(){
	...
	queryOperatorPerson()
	//自动生成出入库单号
	queryMaterial_RK_CK_OrderNoByAuto();
	//查询部门
	//queryDeptNoList();
	getDeptTree();
	//查询仓库
...
function showHide(){
	...
	$("#printInsertMaterial_SDData").hide();
	$("#save").hide();
	var list=[];
	arrList.forEach(item=>{
		if(item.ModelCode=="14003"){
			list.push(item);
		}
	...
	for(var item of list){
		if(item.Code=="print"){
			$("#printInsertMaterial_SDData").show();
		}
		if(item.Code=="add"){
			$("#save").show();
		}
	...
	/*$.ajax({
		...
		success:function(data){
			if(data.flag){
				...
				$("#operator").append(html);
				$(".caozuo").html(data.result.holdername);
			}else{
...
//自动生成出入库单号
function queryMaterial_RK_CK_OrderNoByAuto(){
	$.ajax({
 		url:url+"/Material_SDData/queryMaterial_RK_CK_OrderNoByAuto",
		type:"POST",
		success:function(data){	
			$("#in-sdbill").val(data.in_sdbill);
		}
 	})
}
...
$("#save").click(function(){
	...
	insertMaterialSDDataBatch(sdstatus,statusname,sdbill);
})

 
//点击保存，提交入库
function insertMaterialSDDataBatch(sdstatus,statusname,sdbill){
	...
	if(arr.length==0){
		layer.msg("入库信息请填写完整,谢谢！",{time:2000});
		return;
	}
	$.ajax({
		url:url+"/Material_SDData/insertMaterialSDDataBatch",
		type:"POST",
...
$(".itemcode").bind("keypress",function(event){
    ...
    if(event.keyCode == "13")
    {
    	queryMaterialData(itemcode,this);
    }
});

//根据入库页面中的原材料条码查询数据
function queryMaterialData(itemcode,that){
	...
	$.ajax({
		url:url+"/MaterialData/queryMaterialData",
		type:"POST",
		data:obj,
		success:function(data){
			if(!data.flag){
...
$(document).on("dblclick",".itemcode",function(){
	layer.open({
		type:2,
		content:"b_SparePartsData_Layer.html",
		area:["90%","91%"]
	})
})
...
function createBPTable(arr){
	for(var k=0;k<arr.length;k++){
		var YCLCode=arr[k].YCLCode;
		var YCLName=arr[k].YCLName;
		var YCLSpec=arr[k].YCLSpec;
		var YCLType=arr[k].YCLType;
		var YCLUnitName=arr[k].YCLUnitName;
		var YCLItemType=arr[k].YCLItemType;
		var YCLremark=arr[k].YCLremark;
		for(var i=0;i<$(".itemcode").length;i++){
			if($(".itemcode").eq(i).val()==""){
			...
			if(i==$(".itemcode").length-1){
				if($(".itemcode").eq(i).val()!=""){
						var $tr=$("<tr></tr>")
						var $td1=$("<td></td>")
						var $td2=$("<td><input type='text' name='itemcode' class='itemcode' autocomplete='off' value='"+BPCode+"'></td>")
						var $td3=$("<td class='itemname'>"+BPName+"</td>")
						var $td4=$("<td><input type='text' name='quantity' class='quantity' autocomplete='off'></td>")
						var $td5=$("<td><input type='text' name='price' class='price' autocomplete='off'></td>")
						var $td6=$("<td><input type='text' name='sumamount' class='sumamount' autocomplete='off'></td><td class='center'><input type='text'  name='taxrate' class='taxrate' autocomplete='off'></td>")
