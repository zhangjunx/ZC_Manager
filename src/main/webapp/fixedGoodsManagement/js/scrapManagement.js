...
function getList(){
	...
	$.ajax({
		url:url+"/zcGoodsWaring/getScrapList",
		type:"post",
		data:obj,
		async:false,
		success:function(data){
			...
			if(data.flag){
				...
				for(var item of res){
					...
					var operateDate=item.waringDate==undefined?"":item.waringDate;
					...
					var $tr=$("<tr><td>"+index+"</td><td>"+typeName+"</td><td>"+goodsName+"</td><td>"+labelCode+"</td><td>"+setWaringPerName+"</td>" +
							"<td>"+warningLocal+"</td><td>"+operateDate+"</td><td>"+model+"</td><td>"+brand+"</td>" +
