		...
		success:function(data){
			...
			if(data.flag){
				for(var item of data.result){
					...
					var $opt1=$("<option data-brand="+brand+" data-goodsname="+goodsName+" data-goodscode="+item.goodsCode+"  data-specs="+specs+" data-size="+size+" value="+item.fid+">"+model+"</option>")
...
function saveInStorage(){
	...
	for(var i=0;i<$("td[data-field='typeName']").length;i++){
		...
		var projectNo=$("td[data-field='projectNo']").eq(i).find(".layui-table-cell").html();
		var projectName=$("td[data-field='projectName']").eq(i).find(".layui-table-cell").html();
...
$("#createInStorage").click(function(){
	...
	var goodsID=$("#model option:selected").attr("value");//物品id
	var goodsCode=$("#model option:selected").attr("data-goodscode");//物品编码
	var goodsName=$("#model option:selected").attr("data-goodsname");//物品名称
	...
	for(var i=0;i<$("#fileBox a").length;i++){
		var path=$("#fileBox a").eq(i).attr("href");
		var filename=$("#fileBox a").eq(i).html();
		if(i == $("#fileBox a").length -1){
			pathstr+=path;
	...
	layui.use('table', function(){
		  ...
		  table.render({
		    ...
		    cols: [[//标题栏
		      ...
		      {field:'fileName', width:120, title: '设备技术规范'},
//		      {field:'size', width:80, title: '图片'},
		      ...
		      {field:'projectNo', width:100 ,title: '项目编号'},
