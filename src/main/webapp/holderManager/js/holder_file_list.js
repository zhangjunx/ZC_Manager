...
function getList(){//查询用户列表
	...
	var obj={"holderno":holderno,"holdername":holdername,
			"idcode":idcode,"pageIndex":page,"pageSize":limit};
	var str=window.top.holderno;
	$.ajax({
		url:url+"/ZX_HolderFile/getHolderFileListByPage?str="+str,
		type:"POST",
		data:obj,
		dataType:"json",
		async:false,//很重要
			...
			if(!data.flag){
				...
				var text2="";
				 if(list.findIndex(target=>target.Code=="query")==-1&&window.top.arr.length!=0){
					 ...
					 text="<a href='javascript:;' class='layui-btn layui-btn-xs layui-btn-warm see'>查看</a>";
				 }
				 if(list.findIndex(target=>target.Code=="update")==-1&&window.top.arr.length!=0){
					 text1="";
				  }else {
					 text1="<a href='javascript:;' class='layui-btn layui-btn-xs mo'>修改</a>";
				  }
				...
				$.each(data.result,function(i,val){
					...
					var stay=(val.stay=="0"?"住宿":"未住宿");
					var floorroom=(val.floorroom==undefined?"":val.floorroom);
					var medicalInsure=(val.medicalInsure=="0"?"已缴纳":"未缴纳");
					var endinsure=(val.endinsure=="0"?"已缴纳":"未缴纳");
					var injuryinsure=(val.injuryinsure=="0"?"已缴纳":"未缴纳");
					var birthinsure=(val.birthinsure=="0"?"已缴纳":"未缴纳");
					var unemployinsure=(val.unemployinsure=="0"?"已缴纳":"未缴纳");
					var housefund=(val.housefund=="0"?"已缴纳":"未缴纳");
					var updatepersonname=(val.updatepersonname==undefined?"":val.updatepersonname);
					var updatedate=(val.updatedate==undefined?"":val.updatedate);
					
					if(idcode.length==18){
					    idcode=idcode.replace(idcode.substring(6,14),"********");	
					}
					html+="<tr><td class='no-print'><span class='checkbox' style='float:none'></span></td><td>"+holderno
						+"</td><td>"+holdername
						+"</td><td>"+idcode
						+"</td><td>"+stay
						+"</td><td>"+floorroom
					...
					+"</td><td>"+injuryinsure
					+"</td><td>"+birthinsure
					+"</td><td>"+unemployinsure
					+"</td><td>"+housefund
					+"</td><td>"+updatepersonname
					+"</td><td>"+updatedate
					+"</td><td class='center no-print'>"+text+text1+text2+"</td></tr>";
				})
				$("#cont").append(html);
		}
	})
...
$(document).on("click",".shou",function(){
	...
	layer.confirm("确定修改?",{title:"提示"},function(index){
		...
		$.ajax({
			url:url+"/ZX_HolderFile/updateHolderStatus",
			type:"POST",
			data:obj,
			dataType:"json",
			success:function(data){
...
$(".bottom_sure").click(function(){
	...
	var obj={"holderno":holderno,"leavetype":leavetype,"leavetypename":leavetypename,
			"expiredate":expiredate,"leavereason":leavereason,"loginholderno":loginholderno};
	console.log(obj)
	$.ajax({
...
$("#export").click(function(){
	layer.confirm("选择导出数据",{
		...
		btn2:function(index,layero){
			...
			var str=window.top.holderno;
			$.ajax({
				url:url+"/ZX_HolderFile/getHolderFileList?str="+str,
				type:"post",
				success:function(data){
					layer.close(index);
					if(data.flag){
...
function deleteHolderDataBatch(){
	...
	layer.confirm("确定删除?",{title:"提示"},function(index){
		layer.close(index);
		$.ajax({
			url:url+"/ZX_HolderFile/updateHolderDataBatchs",
			type:"POST",
