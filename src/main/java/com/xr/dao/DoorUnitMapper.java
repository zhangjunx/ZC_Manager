...
@Repository
public interface DoorUnitMapper {
	

    int insert(DoorUnit record);
    ...
    int updateByPrimaryKey(DoorUnit record);
    List<DoorUnit>  queryDoorUnitList();
    List<Map<String,Object>> queryDoorAndDeviceList();//查询门区及所在控制器列表 门禁权限设置时用到
    List<Map<String,Object>> queryAreaAndDoorList();//查询区域和对应门区及所在控制器列表 门禁权限设置时用到
    List<Map<String,Object>> getDoorTree();//门区树
    
    List<DoorUnit> queryDoorListByControl(Map m);//查询访客能进门区列表
    
    List<DoorUnit> queryDoorByDoorNo(Short doorno);//根据门区号查名称
