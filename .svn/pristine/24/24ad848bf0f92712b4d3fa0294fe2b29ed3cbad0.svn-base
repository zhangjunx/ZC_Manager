...
function addScrapList(){
	...
	console.log(arr)
	$.ajax({
		url:url+"/zcGoodsWaring/addRetireList",
		type:"post",
		...
		success:function(data){
			...
			if(data.flag){
				layer.msg(data.reason,{time:1000},function(){
					window.location.href="retireManagement.html";
				})
			}else{
...
function createTable(val,warningLocal){
	...
	var $tr=$("<tr><td>"+len+"</td><td>"+goodsType+"</td><td>"+goodsName+"</td><td class='labelCode'>"+labelCode+"</td>" +
			...
			"<td>"+scrapDate+"</td><td class='warningLocal'>"+warningLocal+"</td><td><a href='javascript:;' class='shan layui-btn layui-btn-xs layui-btn-danger'>删除</a></td></tr>")
	$("#cont").append($tr);
	$("#labelCode").val("");
}
