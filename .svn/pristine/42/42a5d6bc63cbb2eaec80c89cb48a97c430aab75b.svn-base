...
function showHide(){
	...
	$("#printOutMaterial_SDData").hide();
	$("#save1").hide();
	var list=[];
	arrList.forEach(item=>{
		if(item.ModelCode=="14004"){
			list.push(item);
		}
...
function queryMaterial_RK_CK_OrderNoByAuto(){
	$.ajax({
		...
		success:function(data){	
			...
			$("#out-sdbill").val(data.out_sdbill);
		}
 	})
}
...
function insertMaterialSDDataBatch(sdstatus,statusname,sdbill){
	...
	$.ajax({
		url:url+"/Material_SDData/outMaterialSDDataBatch",
		type:"POST",
...
function createTr(index){//创建表格行
	...
	var $td5=$("<td><input type='text' name='price' class='price' autocomplete='off' readonly></td>");
	var $td6=$("<td><input type='text' name='sumamount' class='sumamount' autocomplete='off' readonly></td>");
...
$(".itemcode").bind("keypress",function(event){
    ...
    if(event.keyCode == "13")
    {
    	...
    	queryMaterialSDStockByItemCodeAndWareCode(itemcode,warecode,this);
    }
});
//根据入库页面中的原材料条码查询数据
function queryMaterialSDStockByItemCodeAndWareCode(itemcode,warecode,that){
	...
	console.log(obj)
	$.ajax({
		url:url+"/Material_SDData/queryMaterialSDStockByItemCodeAndWareCode",
		type:"POST",
		data:obj,
		dataType:"json",
		success:function(data){
