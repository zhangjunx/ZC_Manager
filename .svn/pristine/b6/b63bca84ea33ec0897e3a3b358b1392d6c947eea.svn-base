...
function getList(){
	...
	$.ajax({
		url:url+"/zcInStore/getInStoreRecord",
		type:"post",
		data:obj,
		async:false,
		success:function(data){
			...
			if(data.flag){
				...
				for(var item of res){
					...
					var $tr=$("<tr><td><span class='checkbox' data-bill="+bill+" style='float:none'></span></td><td>"+index+"</td><td>"+bill+"</td><td>"+remark+"</td><td>"+operatorName+"</td><td>"+inDate+"</td><td>"+approverName+"</td>" +
							"<td><a href='itemInStorageRecord.html?fid="+item.fid+"&bill="+item.bill+"' class='mo layui-btn layui-btn-xs'>详情</a></td></tr>");
					index++;
					$("#cont").append($tr);
				}
			}
