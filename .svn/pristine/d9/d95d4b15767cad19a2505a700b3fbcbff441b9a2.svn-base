$(function(){
	...
	queryOperatorPerson();
	//自动生成出入库单号
	querySpareParts_RK_CK_OrderNoByAuto();
	//查询部门
	//queryDeptNoList();
	getDeptTree();
	//查询仓库
...
function showHide(){
	...
	$("#save1").hide();
	$("#printOutSpareParts_SDData").hide();
	var list=[];
	arrList.forEach(item=>{
		if(item.ModelCode=="13004"){
			list.push(item);
		}
	...
	for(var item of list){
		...
		if(item.Code=="print"){
			$("#printOutSpareParts_SDData").show();
		}
	}
}//end
...
function queryOperatorPerson(){
	...
	$(".caozuo").append(window.top.holdername);
	/*$.ajax({
		url:url+"/HolderData/queryHolderData",
		type:"POST",
		success:function(data){
			if(data.flag){
				var html="<option value='"+data.result.holderno+"'>"+data.result.holdername+"</option>";
				$("#operator").append(html);
				$(".caozuo").append(data.result.holdername);
			}else{
...
//自动生成入库单号
function querySpareParts_RK_CK_OrderNoByAuto(){
	$.ajax({
 		url:url+"/SpareParts_SDData/querySpareParts_RK_CK_OrderNoByAuto",
		type:"POST",
		success:function(data){	
...
$("#save1").click(function(){
	...
	for(var i=0;i<$(".itemcode").length;i++){
		if($(".itemcode").eq(i).val()!=""){
			...
			$("#product").append($tr);
			//$(".bumen").append($("#departmentno option:selected").html());
		}
	}
	insertSparePartsSDDataBatch(sdstatus,statusname,sdbill);
});
//点击保存，提交入库
function insertSparePartsSDDataBatch(sdstatus,statusname,sdbill){
...
if(arr.length==0){
	layer.msg("请填入完整信息，谢谢！",{time:2000});
	return;	
}
$.ajax({
		url:url+"/SpareParts_SDData/outSparePartsSDDataBatch",
		type:"POST",
...
function createTr(index){//创建表格行
	var $tr=$("<tr></tr>")
	var $td1=$("<td></td>")
	var $td2=$("<td><input type='text' name='itemcode' class='itemcode'></td>")
	var $td3=$("<td class='itemname'></td><td class='storage'></td>")
	var $td4=$("<td><input type='text' name='quantity' class='quantity'></td>")
	var $td5=$("<td><input type='text' name='price' class='price' readonly></td>")
	var $td6=$("<td><input type='text' name='sumamount' class='sumamount' readonly></td>")
...
function queryWareCodeList(){//查询仓库列表
	 $.ajax({
...
$(".itemcode").bind("keypress",function(event){
    ...
    if(event.keyCode == "13")
    {
    	...
    	$(this).parent().parent().children().find(".quantity").val("");
    	$(this).parent().parent().children().find(".price").val("");
    	$(this).parent().parent().children().find(".sumamount").val("");
    	$(this).parent().siblings().eq(6).html("");
    	$(this).parent().siblings().eq(7).html("");
    	$(this).parent().siblings().eq(8).html("");
    	$(this).parent().siblings().eq(9).html("");
    	$(this).parent().siblings().eq(10).html("");
    	if(warecode==""){
    		layer.msg("请选择仓库!",{time:2000});
    		return;
    	}
    	querySparePartsSDStockByItemCodeAndWare(itemcode,warecode,this);
    }
});
//出库时   根据仓库和条码查库存量
function querySparePartsSDStockByItemCodeAndWare(itemcode,warecode,that){
	
	var obj={"itemcode":itemcode,"warecode":warecode};
	//console.log(obj)
	$.ajax({
		url:url+"/SpareParts_SDData/querySparePartsSDStockByItemCodeAndWare",
		type:"POST",
		data:obj,
		dataType:"json",
		success:function(data){
...
$(document).on("dblclick",".itemcode",function(){
	layer.open({
		type:2,
		content:"b_stock_layer.html",
		area:["90%","91%"]
	})
})

function createBPTable(arr){
	for(var k=0;k<arr.length;k++){
		var BPCode=arr[k].BPCode;
		var BPName=arr[k].BPName;
		var BPSpec=arr[k].BPSpec;
		var BPType=arr[k].BPType;
		var BPUnitName=arr[k].BPUnitName;
		var BPStorage=arr[k].BPStorage;
		var BPPrice=arr[k].BPPrice;
		var BPSummoney=arr[k].BPSummoney;
		var BPItemType=arr[k].BPItemType;
		var BPremark=arr[k].BPremark;
		for(var i=0;i<$(".itemcode").length;i++){
			if($(".itemcode").eq(i).val()==""){
				$(".itemcode").eq(i).val(BPCode);
				$(".itemname").eq(i).html(BPName);
				$(".storage").eq(i).html(BPStorage);
				$(".quantity").eq(i).val(BPStorage);
				$(".price").eq(i).val(BPPrice);
				$(".sumamount").eq(i).val(BPSummoney);
				$(".spec").eq(i).html(BPSpec);
				$(".type").eq(i).html(BPType);
				$(".unitname").eq(i).html(BPUnitName);
				$(".itemtype").eq(i).html(BPItemType);
				$(".remark").eq(i).html(BPremark);
				break;
			}
			if(i==$(".itemcode").length-1){
				if($(".itemcode").eq(i).val()!=""){
						var $tr=$("<tr></tr>")
						var $td1=$("<td></td>")
						var $td2=$("<td><input type='text' name='itemcode' class='itemcode' value='"+BPCode+"'></td>")
						var $td3=$("<td class='itemname'>"+BPName+"</td><td class='storage'>"+BPStorage+"</td>")
						var $td4=$("<td><input type='text' name='quantity' class='quantity' value='"+BPStorage+"'></td>")
						var $td5=$("<td><input type='text' name='price' class='price' readonly value='"+BPPrice+"'></td>")
						var $td6=$("<td><input type='text' name='sumamount' class='sumamount' readonly value='"+BPSummoney+"'></td>")
						var $td7=$("<td class='spec'>"+BPSpec+"</td>")
						var $td8=$("<td class='type'>"+BPType+"</td>")
						var $td9=$("<td class='unitname'>"+BPUnitName+"</td>")
						var $td10=$("<td class='itemtype'>"+BPItemType+"</td>")
						var $td11=$("<td class='remark'>"+BPremark+"</td>")
						//var $td12=$("<td><select class='areacode' name='areacode'><option value=''>请选择</option></select></td>")
						$td1.append(index+1)
						$tr.append($td1)
						$tr.append($td2)
