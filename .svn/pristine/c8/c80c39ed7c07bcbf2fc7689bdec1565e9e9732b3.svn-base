...
function getList(){
	...
	$.ajax({
		url:url+"/Material_SDData/queryMaterialSDStockList",
		type:"POST",
		data:obj,
		dataType:"json",
		async:false,
		success:function(data){
			...
			if(!data.flag){
				...
				$.each(data.result,function(i,val){
					...
					html+="<tr><td>"+(i+1)+"</td><td>"+itemcode+"</td><td>"+itemname+"</td><td>"+spec
					+"</td><td>"+type+"</td><td>"+unitname+"</td><td>"+storage1+"</td><td>"
					+storage2+"</td><td>"+storage
...
$("#export").click(function(){
	layer.confirm("选择导出数据",{
		...
		yes:function(index,layero){
		    ...
		    openDownloadDialog(sheet2blob(sheet),'原材料出入库统计.xlsx');
		...
		btn2:function(index,layero){
			...
			$.ajax({
				...
				success:function(data){
					...
					if(data.flag){
						var res=data.result;
						var $table = $("<table id='tableData' style='display:none'><tr><td>序号</td><td>材料编码</td><td>材料名称</td><td>规格</td>" +
					      ...
					      for(var val of res){
					    	  var datano=(val.datano==undefined?"":val.datano);
								...
								insertStorageNumber+=parseFloat(storage1);
			   				    outStorageNumber+=parseFloat(storage2);
			   				    storageNumber+=parseFloat(storage);
			 					var $tr=$("<tr><td>"+(i+1)+"</td><td>"+itemcode+"</td><td>"+itemname+"</td><td>"+spec
										+"</td><td>"+type+"</td><td>"+unitname+"</td><td>"+storage1+"</td><td>"
										+storage2+"</td><td>"+storage
										+"</td><td>"+price+"</td><td>"+sum
										+"</td><td>"+itemtype+"</td><td>"+warename
