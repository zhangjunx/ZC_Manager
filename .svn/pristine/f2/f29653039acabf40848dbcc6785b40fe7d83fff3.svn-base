$(function(){
	//编辑查询
	queryMaterialSDData(localStorage.datano);
});

//点击确定提交
$("#updateMaterialSDDataBtn").click(function(){
	updateMaterialSDData();
})
function updateMaterialSDData(){
	...
	/*var res={"orderno":orderno,"itemcode":itemcode,"itemname":itemname,"quantity":quantity,"sumamount":sumamount,
			"operator":operator,"sdpersonname":sdpersonname,"warename":warename,"areaname":areaname,"companyname":companyname,"spec":spec,"type":type,"unitname":unitname,
...
//编辑查询
function queryMaterialSDData(text){
	var obj={"datano":text};
	$.ajax({
		url:url+"/Material_SDData/queryMaterialSDData",
		type:"POST",
		data:obj,
		dataType:"json",
		success:function(data){
